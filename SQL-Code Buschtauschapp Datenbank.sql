...
-- Erstellen der Tabellen und Einfügen der Dummy Daten --
...
-- Code für die Testfälle --

-- Testfall für Suche nach Genre --
SELECT titel FROM Buecher WHERE genre = "Fantasy" ;

-- Testfall für Suche über Autorenname --
SELECT 
    Buecher.titel
FROM 
    buchtausch_app.Buecher
JOIN 
    buchtausch_app.Autor ON Buecher.FK_autor_ID = Autor.autor_ID
WHERE 
    CONCAT(Autor.autor_name, ' ', Autor.autor_nachname) = 'Christopher Paolini';

-- Testfall für Suche nach Erscheinungsdatum --
  SELECT 
    Buecher.titel, 
    YEAR(Buecher.erscheinungsdatum) AS erscheinungsjahr
FROM 
    buchtausch_app.Buecher
WHERE 
    YEAR(Buecher.erscheinungsdatum) > 2010;

  -- Testfall um Nutzer mit Standortfreigabe anzuzeigen-- 
   SELECT 
    Benutzer.nutzervorname, 
    Benutzer.nutzernachname, 
    Standortfreigabe.freigabe_beschreibung
FROM 
    buchtausch_app.Benutzer
JOIN 
    buchtausch_app.Standortfreigabe ON Benutzer.FK_freigabe_ID = Standortfreigabe.freigabe_ID;

   -- Testfall für Reservierung
   -- update des buch status falls reservierung aktiv --
 UPDATE buchtausch_app.Buecher
SET buch_status = 'reserviert'
WHERE buch_ID IN (
    SELECT FK_buch_ID
...
INSERT INTO buchtausch_app.Reservierung (reservierung_ID, reservierungsdatum, res_startdatum, res_enddatum, res_status, FK_buch_ID)
Values
(4, '2024-09-07', '2024-09-09', '2024-09-30', 'offen', 17);

SELECT buch_id, buch_status FROM Buecher WHERE buch_ID =17;


-- Testfall für neuen Ausleihvorgang -- 
 -- update des buch status falls ausleihvorgang aktiv --
   UPDATE buchtausch_app.Buecher
SET buch_status = 'ausgeliehen'
WHERE buch_ID IN (
    SELECT FK_buch_ID
...
INSERT INTO buchtausch_app.Ausleihvorgang (leih_vorgang_ID, FK_buch_ID, FK_empfaenger, leih_startdatum, leih_enddatum, leih_status)
		VALUES
		(5, 8, 57, '2024-09-07', '2024-11-06', 'aktiv');
	
SELECT buch_id, buch_status FROM Buecher WHERE buch_ID =8;   

-- Testfall für eine Nutzerbewertung --
INSERT INTO buchtausch_app.Nutzerbewertung (nutzer_bewertungs_ID, FK_bewertungs_empfaenger, FK_bewertungs_verfasser, nutzer_bewertungstext, nutzer_sternebwertung)
		VALUES
		(12, 51, 58, 'Es hat alles total reibungslos funktioniert.', 5);
		
-- Testfall für eine Nutzerbewertung --
	INSERT INTO buchtausch_app.Buchbewertung (buch_bewertung_ID, FK_zustand_ID, FK_buch_ID, buch_bewertungstext, buch_sternebewertung)
		VALUES
		 (11, 2, 17, 'Ein must read für jeden Fantasy-Fan!', 5);
		 
-- Testfall für einen Nachrichtenaustausch --
INSERT INTO buchtausch_app.Nachricht (nachricht_ID, FK_nachricht_empfaenger, FK_nachricht_sender, nachricht_inhalt, gesendet_am)
		VALUES
		(16, 51, 52, 'Hallo! Ich würde gerne das Buch ausleihen.', '2024-09-07 10:15:00'),
		(17, 51, 52, 'Klar, kein Thema! ', '2024-09-07 10:20:00');

-- Create View für verfügbare Bücher --
CREATE VIEW VerfuegbareBuecher AS
SELECT 
    B.titel AS Buchtitel,
    A.autor_nachname AS Autor,
    AD.ort AS Ort
FROM 
    buchtausch_app.Buecher B
JOIN 
    buchtausch_app.Autor A ON B.FK_autor_ID = A.autor_ID
JOIN 
    buchtausch_app.Benutzer U ON B.FK_benutzer_ID = U.benutzer_ID
JOIN 
    buchtausch_app.Adresse AD ON U.benutzer_ID = AD.FK_benutzer_ID
WHERE 
    B.buch_status IN ('verfuegbar');
